...
{
    private StressAnalysisForm parentForm;
    ...
    public DensitySettingsForm(StressAnalysisForm parent, MainForm mainForm)
    {
    ...
    private void InitializeControls()
    {
        ...
        KryptonGroupBox densityGroup = new KryptonGroupBox
        {
            ...
            Size = new Size(520, 70) // Increased height to ensure content visibility
        };
        ...
        {
            Text = "Density (kg/m³):",
            Location = new Point(20, 15) // Positioned properly inside panel
        };

        numDensity = new KryptonNumericUpDown
        {
            Location = new Point(150, 15), // Aligned with label
            Width = 100,
            Minimum = 0.1M,
            Maximum = 20000M,
            DecimalPlaces = 1,
        ...
        rbCalibration = new KryptonRadioButton
        {
            ...
            Location = new Point(20, 260) // Positioned after density group
        };
    ...
    private void StartRegionSelection()
    {
        ...
        // Show instructions
        MessageBox.Show("Click and drag in the XY view to select a region for calibration.",
                       "Region Selection", MessageBoxButtons.OK, MessageBoxIcon.Information);

        // Add event handlers to the main form's XY view for region selection
        mainForm.xyView.MouseDown += RegionSelect_MouseDown;
        mainForm.xyView.MouseMove += RegionSelect_MouseMove;
        mainForm.xyView.MouseUp += RegionSelect_MouseUp;

        // Set cursor to crosshair
        mainForm.xyView.Cursor = Cursors.Cross;
    }
    ...
    private void CancelRegionSelection()
    {
        ...
        // Reset cursor
        mainForm.xyView.Cursor = Cursors.Default;
    ...
    private void RegionSelect_MouseMove(object sender, MouseEventArgs e)
    {
        if (isDrawingSelection)
        {
            ...
            mainForm.xyView.Invalidate(); // Force repaint
        }
    }

    private void RegionSelect_MouseUp(object sender, MouseEventArgs e)
    {
        if (e.Button == MouseButtons.Left && isDrawingSelection)
        {
            selectionEnd = e.Location;
            isDrawingSelection = false;

            // Calculate the rectangle in image coordinates
            Point imgStart = ConvertToImageCoordinates(selectionStart, mainForm.xyPan, mainForm.xyZoom);
            Point imgEnd = ConvertToImageCoordinates(selectionEnd, mainForm.xyPan, mainForm.xyZoom);
            ...
            // Calculate average gray value in the region
            double avgGrayValue = CalculateRegionAvgGrayValue(x, y, width, height);

            // Process the selected region
            ProcessSelectedRegion(new Rectangle(x, y, width, height), avgGrayValue);
    ...
    private Point ConvertToImageCoordinates(Point screenPoint, PointF pan, float zoom)
    {
        return new Point(
            (int)((screenPoint.X - pan.X) / zoom),
            (int)((screenPoint.Y - pan.Y) / zoom));
    }

    private double CalculateRegionAvgGrayValue(int x, int y, int width, int height)
    {
        ...
        for (int cy = y; cy < y + height; cy++)
        {
            ...
            {
                if (cx >= 0 && cx < mainForm.GetWidth() &&
                    cy >= 0 && cy < mainForm.GetHeight())
                {
                    total += mainForm.volumeData[cx, cy, z];
                    count++;
                }
            }
        }
    ...
    private void ProcessSelectedRegion(Rectangle region, double avgGrayValue)
    {
        if (comboMaterials.SelectedItem is MaterialDensity material)
        {
            CalibrationPoint point = new CalibrationPoint
